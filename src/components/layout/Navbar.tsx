  ...
  return (
    <>
      <nav className={`absolute top-0 left-0 w-full z-[100] h-20 md:h-24 flex items-center transition-all ${
        isHome ? "bg-transparent" : "bg-white border-b border-black/5"
      }`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 w-full flex justify-between items-center">
              ...
              <Link 
                ...
                className={`text-[11px] font-bold uppercase tracking-[0.2em] transition-colors ${
                  location.pathname === link.path 
                    ? "text-safety" 
                    : isHome ? "text-white/90 hover:text-safety" : "text-industrial/70 hover:text-safety"
                }`}
              >
                {link.name}
              </Link>
          ...
          <button className={`md:hidden relative z-[110] ${isHome ? "text-white" : "text-industrial"}`} onClick={() => setIsOpen(!isOpen)}>
